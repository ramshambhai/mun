...
var cellScanColumn = 
	[
	 	{ label: 'ID', 			name: 'idofdatarow', 		width: 150,search: true,hidden:true },
		...
		{ label: 'RSCP', 		name: 'RSCP', 		width: 150,search: true,formatter:function(cellvalue, options, rowObject)
			{
			if(cellvalue != null && cellvalue !="" && cellvalue != undefined && !isNaN(cellvalue))
			{
				return Math.ceil(parseFloat(cellvalue));
			}
			else
			{
				return "";
		...
		{ label: 'RSSI', 		name: 'RSSI', 		width: 150,search: true,formatter:function(cellvalue, options, rowObject)
			{
			if(cellvalue != null && cellvalue !="" && cellvalue != undefined && !isNaN(cellvalue))
			{
				return Math.ceil(parseFloat(cellvalue));
...
var createCellScanReportGridData = function(data){
	...
	//{
		...
		for(var j in data[i].REPORT)
		{
			...
			data[i].REPORT[j].CELL_ID = data[i].REPORT[j].hasOwnProperty('CELL_ID')?data[i].REPORT[j].CELL_ID:"";
			...
			data[i].REPORT[j].PSC = data[i].REPORT[j].hasOwnProperty('PSC')?data[i].REPORT[j].PSC:"";
			...
			if(data[i].REPORT[j].hasOwnProperty('PLMN'))
			{
				...
				var jj = data[i].REPORT[j].PLMN;
				
				for(var kk in jj)
				{
					if(jj[kk].hasOwnProperty('MCC'))
					{
						data[i].REPORT[j].MCC = jj[kk].MCC;
						data[i].REPORT[j].MNC = jj[kk].MNC;
					}
					else
					{
						data[i].REPORT[j].MCC = "";
...
var registerCellscanEvent = function() {
    ...
    $("#get_cdr_data").click(function() {
        ...
        $.ajax({
            url: dirPath + "service/netscan/getcellsreport",
            //url:"../resources/test.json",
            data: {
...
var getDataExcelExport = function() {
    ...
    var reportNameString = "Cell Scan Data";
...
function getColumnsData() {
    var collnames = {};
	for(var i=0;i<cellScanColumn.length;i++){
	if(cellScanColumn[i].hidden!=true){
	collnames[cellScanColumn[i].label]=cellScanColumn[i].name;
	}
	}
    return collnames;
}