...
createRowsForBtsInfo =function(data)
{
	...
	{	
		...
		"<td>"+data[i].dname+"</td>"+
		'<td><button class="btn-match" onclick="getBtsStatus(\''+data[i].ip+'\','+data[i].b_id+','+data[i].dcode+','+data[i].sytemid+')">Check Status</button></td><td><button class="btn-match" onclick="mesTrigger(1)">MES Start</button><button class="btn-match" onclick="mesTrigger(0)">MES Stop</button><button class="btn-match" onclick="setCellLock(false);">Lock</button><button class="btn-match" onclick="setCellUnLock();">UnLock</button></td></tr>';
