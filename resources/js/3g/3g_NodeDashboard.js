...
createRowsForBtsInfo =function(data)
{
	...
	{	
		...
		"<td>"+data[i].dname+"</td>"+
		'<td><button class="btn-match" onclick="getBtsStatus(\''+data[i].ip+'\','+data[i].b_id+','+data[i].dcode+','+data[i].sytemid+')">Check Staus</button></td></tr>';
		/*+
		'<td><button class="operationButton" onclick="mesTrigger(1)">MES Start</button><button class="operationButton" onclick="mesTrigger(0)">MES Stop</button><button class="operationButton" onclick="setCellLock(false);">Lock</button><button class="operationButton" onclick="setCellUnLock();">UnLock</button></td>';*/
...
function doActionOnGroupClick(nodeLevel,currNode,parentNode){
alert('nodeLevel :'+nodeLevel);
alert('currNode :'+currNode);
alert('parentNode :'+parentNode);
//$('#applicationType').contents().find('#add_sufi_tab').hide();
$('#add_sufi_tab').css('display','none');
}