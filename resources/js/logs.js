...
var parseData = function(data,t_id)
{
	...
	for(var i in data.NEIGH_DATA)
	{
		if(data.NEIGH_DATA[i].STATUS == 2 && data.NEIGH_DATA[i].lat != null && data.NEIGH_DATA[i].lon !=null )
		{
			...
			addMarker([parseFloat(data.NEIGH_DATA[i].lon),parseFloat(data.NEIGH_DATA[i].lat)],"blink_tower.gif",vectorSource,dataaa);
		}
	}
	...
	$.ajax({
		...
		{				
			...
			else
			{
				...
				$("#neigh_form").prepend("<tr><td>PID</td><td>Neg</td><td>Self</td><td style='display:none;'>STATUS</td><td>PLMN</td><td>LAC</td><td>CELL</td><td>Rx</td><td>Latitude</td><td>Longitude</td><td>Distance</td></tr>");
			}				
		},
		error:function()
		{
	...
	//{
		 ...
		 if(latLong.lat != "-1" && latLong.lon != "-1" )
			{	
				...
				tr = '<tr id="'+data["Cell"+i]+'"><td>'+i+'</td><td><input type="checkbox" class="neigh_flag_check"></td><td><input type="checkbox" class="self_flag_check"></td><td style="display:none;">2</td><td>'+data["Cell"+i].PLMN+'</td><td>'+data["Cell"+i].LAC+'</td><td>'+data["Cell"+i].CELL_ID+'</td><td>'+data["Cell"+i].RSSI+'</td><td>'+latLong.lat+'</td><td>'+latLong.lon+'</td><td>'+distance+'</td></tr>';
				$("#neigh_form").append(tr);
		}
		else
		{
			tr = '<tr id="'+data["Cell"+i]+'"><td>'+i+'</td><td><input type="checkbox" class="neigh_flag_check"></td><td><input type="checkbox" class="self_flag_check"></td><td style="display:none;">2</td><td>'+data["Cell"+i].PLMN+'</td><td>'+data["Cell"+i].LAC+'</td><td>'+data["Cell"+i].CELL_ID+'</td><td>'+data["Cell"+i].RSSI+'</td><td>NA</td><td>NA</td><td>Unknown</td></tr>';
			$("#neigh_form").append(tr);
		}		
	//}	
}
  ...
  for(var i=550;i<=2750;i=i+550)
	{
		if($("#lat").val() == "" || $("#lon").val()== "")
		{
			alert("Self Latitude and Longitude is not present");
		}
		else
		{
...
var map = new ol.Map({
        layers: [localMap,imageLayer,neighbourSourceLayer,centerCircleLayer,vectorLayer,axisSourceLayer,detectedCircleLayer],
        target: 'map_div',
        view: new ol.View({
          projection: projection,
...
var drawCircleWithLabel = function(map, ta,center1,color1,labelOfCircle) {
	...
	var CircleStyle = new ol.style.Style({
		...
		fill: new ol.style.Fill({
			...
			text:new ol.style.Text({text:labelOfCircle+"",fill:new ol.style.Fill({color: "#ffffff"})})
	});
...
var addMarker = function(cordinate,imageType,vectorSource1,data)
{
...
var createDataToStoreOnServer = function(data)
{
	...
	var serverData = [];
	var root = "'"+data.t_id+"','"+data.nibIp+"',"+imsi+","+imei+","+data.TA+","+data.MCC+","+data.MNC+","+data.LAC+","+data.CELL+","+data.lat+","+data.lon+",'S'";
	serverData.push(root);
	for(var i in data.NEIGH_DATA)
	{
		if(data.NEIGH_DATA[i].STATUS == 2 )
		{
			root= "'"+data.t_id+"','"+data.nibIp+"',"+imsi+","+imei+",null,"+data.NEIGH_DATA[i].MCC+","+data.NEIGH_DATA[i].MNC+","+data.NEIGH_DATA[i].LAC+","+data.NEIGH_DATA[i].CELL+","+data.NEIGH_DATA[i].lat+","+data.NEIGH_DATA[i].lon+",'N'";
			serverData.push(root);
		}	
	}
...
var getDistinctIMSI = function()
{
	$.ajax({
		url:"../Operations",
		data:{"methodName":"getDistinctIMSI"},
		type:"post",
		success:function(data)
		{
...
var getImsiSelect = function(data)
{
	$("#imsi_number").html("");
	$("#imsi_number").append("<option value='-1'>Select IMSI</option>");
	for(var i in data)
	{
		$("#imsi_number").append("<option value="+data[i]+">"+data[i]+"</option>");
	}
}
...
var registerEvents = function()
{
	$("#get_data").click(function(){
		if($("#imsi_number").val() == "-1" || $(this).val() == "-1" || $("#startTime").val() == "" || $("#endTime").val() == "")
		{
			alert("Please Select IMSI ,Start and End Date");
			return false;
		}
		getScanedDataCenter();
	...
	{
		if($(".select_neigh:checked").length <=0)
		{
			alert("Please Select Neighbours");
			return false;
		}
		else
		{
			var chheckedRows = $(".select_neigh:checked").parent();
			calulateSubscriberLocationOffline(chheckedRows);
		}		
	});
...
var getScannedData =function(data)
{
	$.ajax({
		url:"../Operations",
		data:{
				"methodName":"getScanedData",
				"startDate":toUtcTime($("#startTime").val()),
				"endDate":toUtcTime($("#endTime").val()),
				"imsi":$("#imsi_number").val()
			},
		type:"post",
		success:function(data)
		{
			data = jQuery.parseJSON(data);
			createNeighbourTable(data);
		}	
	});
}
var getScanedDataCenter =function(data)
{
	$.ajax({
		url:"../Operations",
		data:{
				"methodName":"getScanedDataCenter",
				"startDate":toUtcTime($("#startTime").val()),
				"endDate":toUtcTime($("#endTime").val()),
				"imsi":$("#imsi_number").val()
			},
		...
		{
			data = jQuery.parseJSON(data);
			createCenterAndAxis(data);
			centerForDistance =data;
			getScannedData();
		}	
	});
}
...
var createCenterAndAxis = function(data)
{
	if(data.length <=0)
	{
		alert("Slef Lat Lon Not Found");
		return false;
	}
	for(var i=550;i<=2750;i=i+550)
	{
		
		drawCenterCircleTower(map,i,[parseFloat((data[0].lon)),parseFloat(data[0].lat)],i,centerCircleLayerSource);
		
   }
	drawCenterCircleTower(map,2,[parseFloat((data[0].lon)),parseFloat(data[0].lat)],i,centerCircleLayerSource);
	
	map.getView().setCenter([parseFloat((data[0].lon)),parseFloat(data[0].lat)]);
	
	drawLine([parseFloat((data[0].lon)),parseFloat(data[0].lat)],[0,0],vectorSource);
	
	map.getView().setZoom(13.5);

}	

var createNeighbourTable = function(data)
{
	$("#neigh_form").html("");
	
	$("#neigh_form").append("<tr>" +
			"<td><input type='checkbox' value='' id='select_all_ngh'></td>" +
			"<td>ID</td>" +
			"<td>MCC</td>" +
			"<td>MNC</td>" +
			"<td>LAC</td>" +
	...
	{	
		...
		$("#neigh_form").append("<tr style='background:"+background+";color:"+color+";'>" +
				"<td data-rxlvl='"+data[i].rxlvl+"' data-isneorself='"+data[i].isneorself+"' data-log='"+data[i].log+"' data-lat='"+data[i].lat+"' data-lon='"+data[i].lon+"' data-mcc='"+data[i].mcc+"'" +
						" data-mnc='"+data[i].mnc+"' data-lac='"+data[i].lac+"' xdata-cell='"+data[i].cell+"' data-packetid='"+data[i].packetid+"'" +
								" data-nibip='"+data[i].nibip+"' data-imei='"+data[i].imei+"' data-imsi='"+data[i].imsi+"' data-ta='"+data[i].ta+"' data-t_id='"+data[i].t_id+"'>" +
				"<input type='checkbox' class='select_neigh' /></td>" +
						"<td>"+data[i].t_id+"</td>" +
						"<td>"+data[i].mcc+"</td>" +
						"<td>"+data[i].mnc+"</td>" +
						"<td>"+data[i].lac+"</td>" +
						...
						"<td>"+distanceBetweenLatLon(parseFloat(centerForDistance[0].lat),parseFloat(centerForDistance[0].lon),parseFloat(data[i].lat),parseFloat(data[i].lon))+"</td>" +
						"<td>"+data[i].log+"</td>" +
				"</tr>");
		
		drawNeighBourCircleTower(map,10,[parseFloat(data[i].lon),parseFloat(data[i].lat)],data[i].t_id);
	
	
	}
	
	$("#neigh_form").css("height","238px");
	
	$("#select_all_ngh").click(function(){
		$(".select_neigh").trigger("click");
	});
...
var calulateSubscriberLocationOffline = function(calData)
{
	//sum = parseInt(data["RXLVL"]);
	sum = 0;
	
	$(calData).each(function(){
		sum = sum+parseInt($(this).data("rxlvl"));
	});
	
	for(var i in calData)
	{
		
		//sum = sum+parseInt($(calData[i]).data("rxlvl"));
		//consoe.log(calData[i])
	...
	$(calData).each(function(){
		//sum = sum+parseInt($(this).data("rxlvl"));
		$(this).data("work",parseInt($(this).data("rxlvl"))/sum);
	});	
	...
	$(calData).each(function(){
		...
		x=x+$(this).data("work")*parseFloat($(this).data("lat"));
		y=y+$(this).data("work")*parseFloat($(this).data("lon"));
	});
	console.log({"x":x,"y":y});
	//return {"x":x,"y":y};
	
	drawCircleWithLabel(map,8,[y,x],"#FF0000",circleLable);
	
	$("#neigh_form_cal").append("<tr><td>"+circleLable+"</td><td>"+x+","+y+"</td><td>"+distanceBetweenLatLon(parseFloat(centerForDistance[0].lat),parseFloat(centerForDistance[0].lon),parseFloat(x),parseFloat(y))+"</td></tr>")
	circleLable++;
}
