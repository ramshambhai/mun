...
var parseCellData = function(data)
{
	var tr = ""
		$("#neigh_form").html("");
	$("#neigh_form").html("<tr><td></td><td style='display:none;'>STATUS</td><td>PLMN STR</td><td>LAC</td><td>CELL</td><td>Rx Lvl(dBm)</td></tr>");
	for(var i =1;i<=parseInt(data.BSNos);i++)
	{
		tr = '<tr id="'+data["Cell"+i]+'"><td><input type="checkbox" class="neigh_flag_check"></td><td style="display:none;">2</td><td>'+data["Cell"+i].PLMN+'</td><td>'+data["Cell"+i].LAC+'</td><td>'+data["Cell"+i].CELL_ID+'</td><td>'+data["Cell"+i].RSSI+'</td></tr>';
