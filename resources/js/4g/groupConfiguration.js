...
var returnDataCellScan = {"MCC":"","MNC":"","LAC":"","CELL":"","PSC":"","UARFCN":"","PCI":"","EARFCN":"","TAC":""};
...
var getDefaultSufiConfiguration = function(){	
		$.ajax({
			url:dirPath+"service/4g/getdefaultsuficonfiguration",
			async:false,
			type:'GET',
			success:function(data)
			{
			...
			returnDataCellScan = {"MCC":"","MNC":"","LAC":defaultSufiConfig.CELL_PARAMS.SIB_INFO.NEIGH_CELL_LIST.INTER_FREQ[0].LAC,"CELL":"","PSC":"","UARFCN":"","PCI":"","EARFCN":"","TAC":defaultSufiConfig.CELL_PARAMS.SIB_INFO.NEIGH_CELL_LIST.INTER_FREQ[0].TAC,"BAND":""};			
			}
		});
}
...
var getBandDluarfcnMapLTE = function(){	
	$.ajax({
		url:dirPath+"service/4g/getbanddluarfcnmapLTE",
		async:false,
		type:'GET',
		success:function(data)
		{
...
{	
	...
	$('#addHodlSubBtn').on("click",function()
	{
		
		var value = $("#hold_sys_sub_id").val();
		var type=$("#hold_sys_sub_type").val();
		if(value == "" || value.length <14 || value.length >17)
		{
		...
		/*if(subMode=="inclusion")
		{
			...
			else
			{
				...
				{
					if(holdSubscriberInfoNumber==globalTargetImsiList[i]) 
						count++;
				}
			}
			if(count==0)
		...
		addHoldSbscribersIntoList(value,type);
	});
...
var addHoldSbscribersIntoList = function(value,type)
{
	var tr = "";
	if(type == "0")
	{
		tr = "<tr><td><input class='hold_value' type='hidden' value='"+type+"_"+value+"'></td><td>"+value+"</td></tr>";
	}
	else
	{
		tr = "<tr><td><input class='hold_value' type='hidden' value='"+type+"_"+value+"'>"+value+"</td><td></td></tr>";
	}
	$("#displayHoldTargetTable").append(tr);
	
}


var setBandsOfUMTSCellBand = function(){
	$.ajax({
		url:"../../resources/config/band_freq_map.json",
		datetype:"json",
		async:false,
		...
		{
		$('#band2G').html("");
         var options=createGsmBandList(data.UMTS.BAND);
...
function setConfiguration()
{
	...
	{
		alert('DL EARFCN value should be unique');
		return false;
	}
	if(pscOF==pscPPF || pscPPF==pscSPF ||  pscSPF==pscOF)
	{
		alert('PCI value should be unique');
		return false;
	}
	...
	for(var i=0;i<deviceDetails.length;i++)
	{
		if(globalConfigStatus && deviceDetails[i].use_type_name=="4G Locator")
		{
...
$('#inter_rat_2g_sib_table_PPF tbody').html('');
$('#inter_rat_3g_sib_table_PPF tbody').html('');
$('#inter_sib_table_PPF tbody').html('');
$('#intera_sib_table_PPF tbody').find("tr:gt(0)").remove();

$('#inter_rat_2g_sib_table_SPF tbody').html('');
$('#inter_rat_3g_sib_table_SPF tbody').html('');
$('#inter_sib_table_SPF tbody').html('');
$('#intera_sib_table_SPF tbody').find("tr:gt(0)").remove();

$('#inter_rat_2g_sib_table_OF tbody').html('');
$('#inter_rat_3g_sib_table_OF tbody').html('');
...
if(cellScanRowArray[i].TECH=='LTE')
{
	if(cellScanRowArray[i].EARFCN==$('#dlUarfcnPPF').val())
	{
    		row ='<tr class="intera_input_row">'
			    +'<td data-param="PCI" class="intera_opr_td">'+cellScanRowArray[i].PCI+'</td>'
    			...
    			+'<td data-param="TAC" class="inter_opr_td">'+cellScanRowArray[i].TAC+'</td>'
	...
	else
	{
    		row ='<tr class="inter_input_row">'
			    //+'<td data-param="PCI1" class="inter_opr_td"></td>'
    			//+'<td data-param="PCI2" class="inter_opr_td"></td>'
    			//+'<td data-param="DL_Carrier_Frequency" class="inter_opr_td"><input class="inter_text" type="text" value="'+cellScanRowArray[i].FREQ+'"></td>'
    			//+'<td data-param="Priority" class="inter_opr_td"><input class="inter_text" type="text" value="7"></td>'
    			+'<td data-param="DL_Carrier_Frequency" class="inter_opr_td">'+cellScanRowArray[i].FREQ+'</td>'
    			+'<td data-param="Priority" class="inter_opr_td">7</td>'
	...
	if(cellScanRowArray[i].EARFCN==$('#dlUarfcnSPF').val()){
			row ='<tr class="intera_input_row">'
			    +'<td data-param="PCI" class="intera_opr_td">'+cellScanRowArray[i].PCI+'</td>'
				+'<td data-param="CELL_ID" class="intera_opr_td">'+cellScanRowArray[i].CELL_ID+'</td>'
				+'<td data-param="TAC" class="intera_opr_td">'+cellScanRowArray[i].TAC+'</td>'
				+'<td data-param="MCC" class="intera_opr_td">'+cellScanRowArray[i].MCC+'</td>'
				+'<td>';
	...
	if(cellScanRowArray[i].EARFCN==$('#dlUarfcnOF').text()){
		row ='<tr class="intera_input_row">'
		    +'<td data-param="PCI" class="intera_opr_td">'+cellScanRowArray[i].PCI+'</td>'
			+'<td data-param="CELL_ID" class="intera_opr_td">'+cellScanRowArray[i].CELL_ID+'</td>'
			+'<td data-param="TAC" class="intera_opr_td">'+cellScanRowArray[i].TAC+'</td>'
	...
	else
	{
	     ...
	     {
	    	 row ='<tr class="inter_rat_3g_input_row">'
	        		+'<td data-param="DL_Carrier_Frequency" class="inter_rat_opr_td"><input class="inter_rat_text" type="text" value="'+cellScanRowArray[i].FREQ+'"></td>'
	        		+'<td data-param="Priority" class="inter_rat_opr_td"><input class="inter_rat_text" type="text" value="5"></td>'
					...
					+'</tr>';
	    	 	$('#inter_rat_3g_sib_table_PPF tbody').append(row);
				$('#inter_rat_3g_sib_table_SPF tbody').append(row);
				$('#inter_rat_3g_sib_table_OF tbody').append(row);
	     }
	     else
	     {
	    	 
	    	 
	    	 bandindicator = (cellScanRowArray[i].BAND).toUpperCase == "DCS_1800"?0:1;
	    	 row ='<tr class="inter_rat_2g_input_row">'
	    		 	+'<td data-param="Starting_ARFCN" class="inter_opr_td"><input class="inter_text" type="text" value="'+cellScanRowArray[i].ARFCN+'"></td>'
	    		 	+'<td data-param="NCC_Permitted" class="inter_rat_opr_td"><input class="inter_rat_text" type="text" value="1"></td>'
					+'<td data-param="Band_Indicator" class="inter_rat_opr_td"><input class="inter_rat_text" type="text" value="'+bandindicator+'"></td>'
					+'<td data-param="Priority" class="inter_rat_opr_td"><input class="inter_rat_text" type="text" value="5"></td>'
					+'<td data-param="CELL_ID" class="inter_rat_opr_td">'+cellScanRowArray[i].CELL_ID+'</td>'
					+'<td data-param="LAC" class="inter_rat_opr_td">'+cellScanRowArray[i].LAC+'</td>'
					//+'<td data-param="ARFCN1" class="inter_rat_opr_td"><input class="inter_rat_text" type="text" value="1"></td>'
					//+'<td data-param="ARFCN2" class="inter_rat_opr_td"><input class="inter_rat_text" type="text" value="1"></td>'
					+'<td data-param="MCC" class="inter_rat_opr_td">'+cellScanRowArray[i].MCC+'</td>'
					+'<td data-param="MNC" class="inter_rat_opr_td">'+cellScanRowArray[i].MNC+'</td>'
					+'</tr>';
	    	 	$('#inter_rat_2g_sib_table_PPF tbody').append(row);
				$('#inter_rat_2g_sib_table_SPF tbody').append(row);
				$('#inter_rat_2g_sib_table_OF tbody').append(row);
