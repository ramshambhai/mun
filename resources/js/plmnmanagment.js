...
var insertNcellsData = function(cellsData,flag)
{
	$.ajax
	({
		url:"../../service/2g/insertNcellsData",
		type:"post",
		data:{"cellsData":cellsData,"flag":flag},
		dataType:"json",
		success:function(data)
		{
			...
			{
				alert("Unable to update Neighbour Data Please create Network Again");
			}
		}
	});
...
var locateWithNeighbour = function(t_id,neighbourData,dataForInsert)
{
	console.log("asking for request");
	console.log("IMSI"+$("#imsi_number").val());
	flag = 2;
	...
	$.ajax({
		...
		{			
			...
			if(flag == 2)
			{
				...
				insertNcellsData(dataForInsert,2);
				location.reload();				
			}
			if(flag == 3)
			{
				...
				{
					alert("Neighbour Successfully Changed");
					insertNcellsData(dataForInsert,3);
				}				
				location.reload();				
			}
			if(pollManualFlag)
...
$(document).ready(function()
	...
	$('#cancelSearch').click(function(){
		...
		{
			window.location.href="../../views/dashboard/operation.jsp";
		}
		else
		{
			window.location.reload();
	...
	$('#ne_flag').click(function(){	
		if($('#ne_flag').is(":checked"))
		{
			...
			$("#searchWithNegh").css('display','inline');
		}
		else
		{
			//$("#neigh_form").css('display','none');
	...
	$("#searchWithNegh").click(function(){
		...
		var neighSelfSameFlag = false;
		$(".selected").each(function(){
			var nebrSelectedId = $(this).closest("tr").attr("id");
				if(nebrSelectedId == $(".selfisselected").closest("tr").attr("id"))
				{
					
					neighSelfSameFlag = true;
				}
			});
		
		if(neighSelfSameFlag)
		{
			alert("Neighbor PLMN and cell Id can not be same as Self PLMN and cell Id");
			return false;
		...
		if($("#detect_radio").is(":checked"))
		{
			...
			if($(".selected").length >=1 || $(".selfisselected").length >=1)
			//if(true)
			{
					var data = createNeighbourServerData();
					var dataForInsert = createNeighbourServerDataForInsert();
					locateWithNeighbour(null,data,dataForInsert);
			}
			else
			{
...
var getCellsData = function(MSISDN,t_id)
{
	...
	try
	{
		dataopr = jQuery.parseJSON(getOprCellsData());
	}
	catch(err)
	{
		dataopr = [];
	...
	$.ajax({
		...
		{				
			...
			else
			{
				...
				placeCellsMapDataOverToMAp(data,dataopr);
				...
				$("#neigh_form").prepend("<tr><td>PID</td><td>Nbr</td><td>Self</td><td style='display:none;'>STATUS</td><td>PLMN</td><td>LAC</td><td>CELL</td><td>ARFCN</td><td>BSIC</td><td>Rx</td><td>Latitude</td><td>Longitude</td><td>Distance</td><td>CONT</td><td>OPR</td></tr>");
				if(autoFlag)
				{
...
var createNeighbourServerDataForInsert =  function()
{
	var nighbourFlagsCount = 3;
	var  dataForRow = "";	
	$(".selected").each(function(){			
		...
		$(this).find("td").each(function(){
			...
			if( $(this).index() >2 && $(this).index() <10)
			{
				//console.log(nighbourFlagsCount);
				nighbourFlagsCount++;
				tdData += tdData == ""?$(this).text():","+$(this).text();
			}				
		});
		dataForRow += dataForRow == ""?tdData:"$"+tdData;
	});
		...
		if($(this).is(":checked"))
		{
			...
			$(tr).find("td").each(function(){
				...
				if($(this).index() >2 && $(this).index() <10)
				{
					...
					tdData += tdData == ""?value_data:","+value_data;
				}				
			});
...
var parseCellDatawithMap = function(data,i,latLong,distance)
{
	...
	//{
		...
		if(i <=2)
		{
				...
				var name =  (i==1)?"Self":"Configured";
			    tr = '<tr '+style+' id="'+data["Cell"+i]+'"><td>'+i+'</td><td colspan="2">'+name+'</td><td style="display:none;">2</td><td>'+data["Cell"+i].PLMN+'</td><td>'+data["Cell"+i].LAC+'</td><td>'+data["Cell"+i].CELL_ID+'</td><td>'+data["Cell"+i].ARFCN+'</td><td>'+data["Cell"+i].BSIC+'</td><td>'+data["Cell"+i].RSSI+'</td><td>NA</td><td>NA</td><td>'+distance+'</td><td>'+latLong.count+'</td><td>'+latLong.opr+'</td></tr>';
				$("#neigh_form").append(tr);
		}
		else if(latLong.lat != "-1" && latLong.lon != "-1" )
		{	
		
				tr = '<tr id="'+data["Cell"+i]+'"><td>'+i+'</td><td><input type="checkbox" class="neigh_flag_check"></td><td><input type="checkbox" class="self_flag_check"></td><td style="display:none;">2</td><td>'+data["Cell"+i].PLMN+'</td><td>'+data["Cell"+i].LAC+'</td><td>'+data["Cell"+i].CELL_ID+'</td><td>'+data["Cell"+i].ARFCN+'</td><td>'+data["Cell"+i].BSIC+'</td><td>'+data["Cell"+i].RSSI+'</td><td>'+latLong.lat+'</td><td>'+latLong.lon+'</td><td>'+distance+'</td><td>'+latLong.count+'</td><td>'+latLong.opr+'</td></tr>';
				$("#neigh_form").append(tr);
		}
		else
		{
				tr = '<tr id="'+data["Cell"+i]+'"><td>'+i+'</td><td><input type="checkbox" class="neigh_flag_check"></td><td><input type="checkbox" class="self_flag_check"></td><td style="display:none;">2</td><td>'+data["Cell"+i].PLMN+'</td><td>'+data["Cell"+i].LAC+'</td><td>'+data["Cell"+i].CELL_ID+'</td><td>'+data["Cell"+i].RSSI+'</td><td>'+data["Cell"+i].ARFCN+'</td><td>'+data["Cell"+i].BSIC+'</td><td>NA</td><td>NA</td><td>Unknown</td><td>'+latLong.count+'</td><td>'+latLong.opr+'</td></tr>';
				$("#neigh_form").append(tr);
		}		
	//}	
}

var parseCellDatawithMap_new = function(data,i,latLong,distance)
{
		if(data[i].lat != "-1" && data[i].lon != "-1" )
		{	
		
				tr = '<tr id="map_tab_'+data[i].id+'"><td>'+(parseInt(i)+3)+'</td><td><input type="checkbox" class="neigh_flag_check"></td><td><input type="checkbox" class="self_flag_check">|<a class="neg_anch" onclick="getarcnofneighbours('+data[i].packet_id+','+data[i].index_id+','+"'map_tab_"+data[i].id+"'"+')">negh</a>|</td><td style="display:none;">2</td><td>'+data[i].mcc+data[i].mnc+'</td><td>'+data[i].lac+'</td><td>'+data[i].cell+'</td><td>'+data[i].arfcn+'</td><td>'+((parseInt(data[i].bcc))+(parseInt(data[i].ncc)*8))+'</td><td>'+data[i].rssi+'</td><td>'+data[i].lat+'</td><td>'+data[i].lon+'</td><td>'+distance+'</td><td>'+data[i].country+'</td><td>'+data[i].operators+'</td></tr>';
				$("#neigh_form").append(tr);
		}
		else
		{
				//tr = '<tr id="'+data["Cell"+i]+'"><td>'+i+'</td><td><input type="checkbox" class="neigh_flag_check"></td><td><input type="checkbox" class="self_flag_check"></td><td style="display:none;">2</td><td>'+data["Cell"+i].PLMN+'</td><td>'+data["Cell"+i].LAC+'</td><td>'+data["Cell"+i].CELL_ID+'</td><td>'+data["Cell"+i].RSSI+'</td><td>NA</td><td>NA</td><td>Unknown</td><td>'+data["Cell"+i].ARFCN+'</td><td>'+data["Cell"+i].BSIC+'</td><td>'+latLong.count+'</td><td>'+latLong.opr+'</td></tr>';
				//$("#neigh_form").append(tr);
				tr = '<tr id="map_tab_'+data[i].id+'"><td>'+(parseInt(i)+3)+'</td><td><input type="checkbox" class="neigh_flag_check"></td><td><input type="checkbox" class="self_flag_check">|<a class="neg_anch" onclick="getarcnofneighbours('+data[i].packet_id+','+data[i].index_id+','+"'map_tab_"+data[i].id+"'"+')">negh</a>|</td><td style="display:none;">2</td><td>'+data[i].mcc+data[i].mnc+'</td><td>'+data[i].lac+'</td><td>'+data[i].cell+'</td><td>'+data[i].arfcn+'</td><td>'+((parseInt(data[i].bcc))+(parseInt(data[i].ncc)*8))+'</td><td>'+data[i].rssi+'</td><td>NA</td><td>NA</td><td>Unknown</td><td>'+data[i].country+'</td><td>'+data[i].operators+'</td></tr>';
				$("#neigh_form").append(tr);
		}
}
...
var checkBoxClickEvent = function()
{
	...
	$(".self_flag_check").click(function(){
		...
		$(".self_flag_check").each(function(){
			...
			if($(this).is(":checked"))
			{
				...
				$(this).addClass("selfisselected");
				var arfcn = $(this).parent().parent().find("td:eq(7)").text();
				$("#self_arfcn").val(parseInt(arfcn));
				$(this).parent().find(".neg_anch").show();
			...
			else
			{
				...
				$(this).parent().parent().removeClass("selected_neigh");
				$(this).removeClass("selfisselected");
				//$("#self_arfcn").val("");
				$(this).parent().find(".neg_anch").hide();
			}
		});
		if(chekdedCount ==0)
		{
	...
	$("#detect_radio").click(function(){
		...
		$(".self_flag_check").removeAttr("disabled");
	});
	$("#chng_ngh_radio").click(function(){
		...
		if($("#chng_ngh_radio").is(":checked"))
		{
			$(".self_flag_check.selfisselected").trigger("click");
			
			$(".self_flag_check").attr({"disabled":"disabled"});
		}
		else
		{
			$(".self_flag_check").removeAttr("disabled");
	...
	$("#arfcn_modal .modal-body").on("click",".arfcn_self_check_box",function(){
		
		var currentElement = $(this);
		//console.log($(this).is(":checked"));
		//$(".neg_anch").hide();
		$(".arfcn_self_check_box").each(function(){
			var count=0;
			if($(this).is(currentElement))
			{
				...
				if(currentElement.is(":checked"))
				{
					...
					if(!$("#"+$("#curr_negh_input").val()+" .self_flag_check").is(":checked"))
					{
						$("#"+$("#curr_negh_input").val()+" .self_flag_check").trigger("click");
					}
					$("#self_arfcn").val(currentElement.val());
					//$("#"+$("#curr_negh_input").val()).find(".neg_anch").show();
				}
				else
				{
					...
					$("#self_arfcn").val($("#"+$("#curr_negh_input").val()).find("td:eq(7)").text());
...
var placeCellsMapDataOverToMAp = function(data,data2)
{  
  ...
  for(var jj in data2)
  {
	  ...
	  console.log(jj+"-"+data2[jj]);
	    var distance = null;
	    if(data2[jj].lat != "-1" && data2[jj].lon != '-1')
	    {
	    	distance = distanceBetweenLatLon($("#lat").val(),$("#lon").val(),data2[jj].lat,data2[jj].lon)
	    }
	    else
	    {
	    	distance="unkown";
	    }
	    parseCellDatawithMap_new(data2,jj,latLong,distance);
	    if(data2[jj].lat != "-1" && data2[jj].lon != "-1" )
		{			
			drawNeighBourCircleTower(map,10,[parseFloat(data2[jj].lon),parseFloat(data2[jj].lat)],"'"+(parseInt(jj)+3)+"'");
		}
  }
...
var getarcnofneighbours = function(packetId,indexId,rowId)
{
...
$(".self_flag_check").each(function(){
	...
	if(rowId != $("#curr_negh_input").val()){
	
	
	
	var celldata={"index":indexId,"id":packetId,"tech_id":"1"};
	$.ajax({
...
var createArfcnList = function(data,rowId)
{
	
	var jsonData = jQuery.parseJSON(data[0].neigh).INTRA_FREQ_NEIGH;
	
	
	$("#arfcn_modal .modal-body").html("");
	$("#arfcn_modal").modal("show");
	var string = "";
	
	for(var i in jsonData)
	{
		string += "[ARFCN:<input type='checkbox' class='arfcn_self_check_box' value='"+jsonData[i].ARFCN+"' />"+jsonData[i].ARFCN+"],";
	}
	
	$("#arfcn_modal .modal-body").append(string);
	$("#arfcn_modal .modal-body").append("<input type='hidden' id='curr_negh_input' value='"+rowId+"'>");
